...
■ **ver 0.7** (Unrereased)
- 不使用のmkv処理実行時の$radioBoth.Checked分岐を削除（MP4同時出力は $checkMP4Also にて制御中）
- 前回のオプション設定を引き継ぐように仕様変更
  - 出力ファイル形式の選択オプション（MP4同時出力オプションも含む）
  - 入力ファイルと同じフォルダに出力するオプション
  - 出力完了後に出力先フォルダを開くオプション
- MADock_settingsの不要なdot-sourceを削除（guiにて一括読み込みの為）
- 
